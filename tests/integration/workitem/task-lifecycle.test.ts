...
describe("task lifecycle", () => {
  it("creates, assigns, notifies with an acknowledgement and schedules the reminders", async () => {
    ...
    // the workflow instance owns the state; the task row has no status column
    const instance = await withTenantTx(DEPT_CS, (tx) => taskInstance(tx, task.id));
    ...
    // reminders are subscribed against the deadline
    ...
    expect(
      await migratorDb.scheduledJob.count({ where: { subjectType: "task", subjectId: task.id } }),
    ).toBeGreaterThan(0);
  });
  ...
  it("submit is blocked until every required deliverable slot holds a document", async () => {
    ...
    await withTenantTx(DEPT_CS, (tx) => transition(tx, instructor, task.id, "start"));
    const before = await withTenantTx(DEPT_CS, (tx) => actionsFor(tx, instructor, task.id));
    ...
    await expect(
      withTenantTx(DEPT_CS, (tx) => transition(tx, instructor, task.id, "submit")),
    ...
    await withTenantTx(DEPT_CS, (tx) =>
      upload(
        ...
        [{ subjectType: "task", subjectId: task.id, linkRole: "deliverable", slotKey: "report" }],
      ),
    );
    const slots = await withTenantTx(DEPT_CS, (tx) => deliverableStatus(tx, task.id));
    expect(slots).toMatchObject([{ key: "report", required: true, satisfied: true }]);
    const after = await withTenantTx(DEPT_CS, (tx) =>
      transition(tx, instructor, task.id, "submit"),
    );
  ...
  it("review and approve write completedAt only through the workflow, and log every transition", async () => {
    ...
    await withTenantTx(DEPT_CS, (tx) => transition(tx, instructor, task.id, "start"));
    await withTenantTx(DEPT_CS, (tx) => transition(tx, instructor, task.id, "submit"));
    expect(
      (await migratorDb.task.findUniqueOrThrow({ where: { id: task.id } })).completedAt,
    ).toBeNull();
    await withTenantTx(DEPT_CS, (tx) => transition(tx, head, task.id, "review"));
    await withTenantTx(DEPT_CS, (tx) => transition(tx, head, task.id, "approve"));
    const done = await migratorDb.task.findUniqueOrThrow({ where: { id: task.id } });
    ...
    expect(
      await migratorDb.auditEvent.count({ where: { subjectType: "task", subjectId: task.id } }),
    ).toBeGreaterThan(0);
  });
  ...
  it("an audience assignment reaches its members, and listTasks finds them through the group", async () => {
    ...
    expect(others.map((t) => t.id)).not.toContain(task.id);
  });

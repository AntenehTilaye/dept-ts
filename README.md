...
A committee is a standing body of the department, and here it is a record of the `committee` feature whose membership IS a `Group` — which is what makes every grant, thread, audience and notification the platform already has work on it without a line of committee code. Constituting one needs a chair who is on it and the terms of reference attached; from then on the group is open, and the derived grants (`committee_member@committee`, `committee_chair@committee`) follow the membership through the outbox. Standing the committee down closes them; bringing it back reopens them.

What it owes the department is a `committee_report`: a child record under the committee, written by anybody on it, read and then acted on by the head. The point of the process is its last question — the issues the committee could not settle — because taking the report forward turns each of those into a `case` the department has to answer, and approving it closes the committee tasks the report said were finished. The committee and its reports are the kernel's pages throughout; the module adds two panels (who is on it, what became of the issues) and two reports.
...
6. **Reports, projections, widgets and search** — `registerReport`, `registerProjection`, `registerWidget` + a layout entry, and `registerSearchSource` for a subject type of the module's own. All of these are data the framework renders or reads; none of them is a page.
